...
# Initialization of Experiment Basic Configurations
if [ ${RunningNode} = "dgx" ]
then
    TorchThreadsNum=16
    TestEnvThreadsNum=12
elif [ ${RunningNode} = "gpu" ]
then
    TorchThreadsNum=8
    TestEnvThreadsNum=4
else
    TorchThreadsNum=12
    TestEnvThreadsNum=10
fi

TianshouRoot=/work/suj/of4on/tianshou #/home/jsu/Desktop/Projects/Offline4Online/tianshou
Baseline=qrdqn
...
Seed=28980811
LR=0.00005 # 0.00005, 0.0001, 0.0005
...
ExpThreadsConfig="--torch-num-threads ${TorchThreadsNum} --test-env-num-threads ${TestEnvThreadsNum}"
BasicTrainingConfig="${ExpThreadsConfig} --logdir ${LogDir} --lr ${LR} --min-q-weight ${MinQWeight} --epoch ${Epoch} --update-per-epoch ${UpdatePerEpoch} --batch-size ${BatchSize} --test-num ${TestEpisodes} --target-update-freq ${TargetUpdateFreq}"
