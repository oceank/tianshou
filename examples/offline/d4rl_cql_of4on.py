...
def get_args():
    ...
    parser.add_argument(
        "--of4on-dataset-filepath", type=str, default=""
    )
...
def test_cql():
    ...
    print(f"[Create testing environments for evaluation]")
    ...
    # log
    print(f"[Set up the logging system for training process]")
    ...
    def load_of4on_dataset(filepath, buffer_size):
        replay_buffer = ReplayBuffer(buffer_size)
        replay_buffer = replay_buffer.load_hdf5(filepath) # load onto RAM
        return replay_buffer 
     
    if not args.watch:
        print(f"[Loading dataset into a replay buffer for offline learning]")
        if args.of4on_dataset_filepath != "":
            replay_buffer = load_of4on_dataset(args.of4on_dataset_filepath, args.buffer_size)
        else:
            replay_buffer = load_buffer_d4rl(args.expert_data_task)
        
        print(f"[Offline Learning Starts]")
        # trainer
    ...
    print(f"[Evaluation of the final policy]")
    policy.eval()
    test_envs.seed(args.seed)
